...
Therefore we can solve our ILP instance in $f(k)$ time.
...
This equals to creating a constraint by setting the duration of the temporal path along $P_{u,v}$ to equal the value $D_{u,v}$.
There are $f(k)$ different paths $P^i_{u,v}$ from $u$ to $v$ in $G$.
...
This results in producing $f(k)$ variables and constraints for our partial ILP.
...
\begin{proof}
    ...
    It follows that $P$ is a segment.
\end{proof}
...
From the above it follows that there are at most $f(k)$ (namely $O(|U|^2$) undetermined segments in $G'$ after algorithm finishes going through fastest temporal paths among vertices of interest.
...
There are $f(k)$ possible paths of this form, since there are $f(k)$ segments in $G'$.
All together we made $f(k)$ new guesses for the structure of the fastest temporal paths.
Each of the previous partial ILP instances is therefore extended into $f(k)$ new ILP instances.
...
Now we check the matrix $D$ and determine which vertex $z_j \in S^j_{w,z} \setminus \{w,z\}$ 
has the biggest value $D_{v_i,z_j}$.
Let us call this vertex a \emph{split vertex of $v_i$ in the segment $S_{wz}$}.
Denote with $z_{j-1}$ and $z_{j-1}$
