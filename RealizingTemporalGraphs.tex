...
\begin{proof}
    ...
    Let $1<j<i$ (skip if $i=1$) and let $e_\ell\in F_{j,i}$ let $w^i_{\ell'}\in W_i$ be incident with $e_\ell$. Then we add edge $\{v_{j-1}^i,u^\ell_{\ell'}\}$ between $v^i_{j-1}$ and the vertex $u^\ell_{\ell'}$ of the edge selection gadget of color combination $j,i$.
    ...
    Let $e_\ell\in F_{i,i+1}$ let $w^i_{\ell'}\in W_i$ be incident with $e_\ell$. Then we add edge $\{\hat{u}^i_n,u^\ell_{n+\ell'}\}$ between $\hat{u}^i_n$ and the vertex $u^\ell_{n+\ell'}$ of the edge selection gadget of color combination $i,i+1$.

    Let $i<j\le k$ (skip if $i=k$) and let $e_\ell\in F_{i,j}$ let $w^i_{\ell'}\in W_i$ be incident with $e_\ell$. Then we add edge $\{v_{j-1}^i,u^\ell_{n+\ell'}\}$ between $v^i_{j-1}$ and the vertex $u^\ell_{n+\ell'}$ of the edge selection gadget of color combination $i,j$.
...
\begin{itemize}
    ...
    \item vertices $v^i_\ell$ with $\ell\in\{0,1,\ldots,i-1,i+1,\ldots,k\}$ and vertex $\hat{u}^i_n$ (if exists) of each validation gadget.
...
(for the details see guess G-\ref{FPT:guess-splitFromAnotherSegmentAndPaths}). \todo{Two cases.}
...
\begin{enumerate}[(i)]
    \item Vertex $x$ is on a segment $S_{u,v}=(u,v_1,v_2,\dots,v_p,v)$. 
    If $x$ is on a fastest path from $u$ to some other vertex, then we know that the fastest path from $u$ to $x$ travels along the $S_{u,v}$.
    Suppose now that the segment $S_{u,v}$ is not a part of a fastest path from $u$ to some other vertex.
    Denote with $P$ the underlying path of the fastest temporal path from $u$ to $v$, 
    and with $Q$ the fastest temporal path of form $u \leadsto v \rightarrow v_p$ (which we know as we have guessed it).
    ...
    and the fastest temporal paths from $u$ to $v_p, v_{p-1}, \dots, $.
    \todo{Missing}
...
At the beginning we have also guessed the fastest path $z_1 \rightarrow w \leadsto u$ (resp.~$z_p \rightarrow z \leadsto u$).
...
We have two options.
\begin{enumerate}[(i)]
    \item Vertices $x,y$ are in the same segment $S_{u,v} = (u,v_1,v_2, \dots, v_p, v)$. W.l.o.g. suppose that $x$ is closer to $u$ in $S_{u,v}$ than $y$.
    Denote with $x = v_i$ and $y = v_j$, where $i < j$.
    ...
    Then we know exactly the structure of all the fastest paths from $x$ to every vertex in $S_{u,v}$,
    with the exception of path $x \leadsto v_k$.
    \todo{Details}
...
$z_1 \rightarrow w \leadsto u \rightarrow v_1$,
$z_1 \rightarrow w \leadsto v \rightarrow v_p$,
$z_r \rightarrow z \leadsto v \rightarrow v_p$ and
$z_r \rightarrow z \leadsto v \rightarrow v_p$.
...
and vice versa.
Since $y$ is not a vertex of interest it holds that $y \in S_{u,v}=(u, v_1, v_2, \dots, v_p, v)$. Let $y = v_j$, for some $j \in [p]$.
...
And one where the edges of the tree are determined with respect to some undetermined edge incident to $w_i$ (resp.~$w_j$).
...
\begin{enumerate}[(i)]
    ...
    \begin{itemize}
    \item $S_{w,z}$ has no undetermined edge $p$ in the middle, or $p \cap \{x \} = \emptyset$.
    ...
    \item The segment $S_{w,z}$ has an extra undetermined edge $p$. Suppose that $p = x z_{i+1}$.
