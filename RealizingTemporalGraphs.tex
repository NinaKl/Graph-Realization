...
But, this would create $f(n,m)$ guesses, and the running time of the algorithm would be too big.
Therefore, we approach the problem using some special properties, which allow us to create $f(k)$ different guesses.
We solve each guess by creating an ILP instance with $f(k)$ variables and $O(n^c)$ constraints, for some $c \in \mathbb{N}$.
...
Therefore we can solve each of our ILP instances in $f(k)$ time.
...
\begin{enumerate}
    ...
    All in total we create $f(k)$ partial ILP instances, each with $f(k)$ variables and $f(n)$ constraints.
\end{enumerate}
...
This results in producing $f(n,m)$ constraints. We have to guarantee only, that the number of guesses, of faster temporal paths, is in $f(k)$ and not in $f(n)$.


We start the algorithm by guessing the structure of 
the fastest temporal paths (we guess the underlying paths) among all pairs of vertices of $U$. 
...
We now create one partial ILP instance for each guess of fastest temporal paths among the vertices from $U$,
which we later on extend to $f(k)$ final ILP instances, after guessing some extra fastest temporal paths.
From now on, for every pair of vertices $u,v \in U$ we assume that we know exactly what is the underlying path of a fastest temporal path from $u$ to $v$.
...
We describe now how to build one partial ILP instance from a specific list of fastest temporal paths.
...
Let $u, v \in U$ be two vertices of interest. 
We know the underlying path $P_{u,v}$ of a fastest temporal path from $u$ to $v$. 
Suppose that $P_{u,v}$ consist of $\ell$ segments $S_{u_i,v_i}$.
Using \cref{claim:FPT-uv-LabelAlmostalledges} we determine the labels of all but $\ell -1$ edges on $P_{u,v}$, with respect to the value of the first edge.
This results in creating one constraint, by setting the duration of the temporal path along $P_{u,v}$ to be equal to the value $D_{u,v}$.
There are $O(k^k)$ different paths $P^i_{u,v}$ from $u$ to $v$ in $G$.
We now have to iterate through all of them and add the constraint that the duration of $(P^i_{u,v}, \lambda)$ is greater or equal to $D_{u,v}$.
We repeat the above procedure for all pairs of vertices in $U$.
This results in producing $O(k^{k^3})$ variables and constraints to our partial ILP.
...
\end{comment}
\todo[inline]{Write examples for variables and constrains this part produces.}

After finishing the previous step for all pairs of vertices in $U$,
the edges of fastest temporal paths from $u$ to $v$, where $u,v \in U$, are determined with respect to the label of the first edge.
...
Note that these edges are exactly the ones that are not a part of any fastest temporal path.
...
\begin{proof}
    ...
    First, all inner vertices of $P$, \ie vertices $v_i \in  S \setminus \{u,v\}$ are of degree exactly $2$. They are of degree at least $2$ since they are on a path $S$ and are incident to two undetermined edges in  $P$.
    ...
    From all of the above, it follows that $P$ is a segment in $G'$.
\end{proof}
...
\end{claim}
\begin{proof}
   Suppose for the contradiction that there are two undetermined segments among $u,v \in U$.
   Denote them with $S_{u,v}$ and $S'_{u,v}$. 
   Then $C = S_{u,v} \cup S_{u,v}'$ form a cycle in $G'$. Since $G'$ admits no loops, the cycle $C$ is of length at least $3$.
...
From the above it follows that there are at most $O(|U|^2$) undetermined segments in $G'$ after algorithm finishes going through fastest temporal paths among vertices of interest.

We now continue with guessing fastest temporal paths among all vertices in $U^*$, 
which by similar arguing as for vertices in $U$, gives us $O(k^{k^6})$ guesses.
Besides that we also guess fastest temporal paths of the following form.
...
We want to guess the underlying paths of the fastest temporal paths from $v_2$ to $z_2$,
where the first path goes 
from $v_2$, through $u$, through $w$ and then to $z_2$,
and the second one goes
from $v_2$, through $u$, through $z$ and then to $z_2$.
And two similar paths from $v_2$ to $z_{p-1}$.
...
we guess the underlying paths of the fastest temporal paths from $w$ to $v_2$, through $u$ and through $v$.
Besides that we guess the underlying paths of the fastest temporal paths from $v_2$ to $w$ through $u$ and through $v$.
...
Let $S^i_{u,v} = (u=v_1,v_2, \dots, v_p = v)$ be a line segment in $G'$, let us
fix a vertex $v_i \in S^i_{u,v} \setminus \{u,v\}$.
Let 
$S^j_{w,z} = (w=z_1,z_2, \dots, z_r = z)$ be another segments in $G'$.
Note that all inner vertices of $S^j_{w,z}$ (\ie vertices in $S^j_{w,z} \setminus \{w,z\}$)
can be reached from $v_i$ by some path that has to go through $w$ oz $z$.
In fact, this is true for all paths starting in any vertex not in $S^j_{w,z}$.
Therefore, the values of the duration of the fastest path from $v_i$ to vertices on 
the segment
$S^j_{w,z}$ have to increase up to the certain point, when traversing the segment coming through $w$ and coming through $z$.
Namely,
let $z_j \in S^j_{w,z} \setminus \{w,z\}$ be the vertex with the biggest value $D_{v_i,z_j}$,
...
We now have to guess which vertex $w$ or $z$ is on a fastest temporal path from $v_i$ to $z_j$.
Similarly,
all fastest temporal paths starting at $v_i$ have to go either through $u$ or through $v$,
which also gives us two extra guesses for the underlying path of the fastest temporal path from $v_i$ to $z_j$.
Therefore, all together we have $4$ possibilities on how the underlying path of the fastest temporal path from $v_i$ to $z_j$ 
starts and ends.
For a fixed $S^i_{u,v}, v_i$ and $S^j_{w,z}$ we can find the vertex $z_j$ in polynomial time, 
or determine that this kind of vertex does not exist.
We then make four guesses where we determine how the fastest temporal path from $v_i$ to $z_j$ passes through vertices $u,v$ and $w,z$.
We repeat this procedure for all pairs of segments,
which results in producing $O(k^6)$ new guesses.

Before we can use the above information, let us first prove the following properties.
\begin{claim}\label{claim:FPT-unlabeldPaths-01}
    Suppose that
    $S^i_{u,v}, S^j_{w,z}$ are two segments with $ v_i \in S^i_{u,v}$ and $z_j \in S^j_{w,z}$,
    where $z_j$ is a split vertex of $v_i$ in the segment $S^j_{w,z}$.
    Suppose that the fastest temporal path from $v_i$ to $z_j$ travels through vertices $u$ and $w$.
    Then the fastest temporal path from $v_i$ to any other vertex of $S^j_{w,z}$, that is closer to $w$,
...
\end{claim}
\begin{proof}
    Let $z_\ell$ be a vertex of $S^j_{w,z}$, that is closer to $w$ than $z$ in the segment.
    ...
    Note that $P_{v_i,z_j}^\ell$ and $P^2_{v_i,z_\ell}$ intersect on a segment $S_{w,z}^j$ from the vertex $w$ to $z_ell$.
    Therefore since
...
We already studied the case of fastest paths starting and ending in a vertex of $U$,
so now we have to study all other cases.

Let us start with the case of determining the fastest path from $u$ to $x$, where $u \in U$ and $x \in V(G') \setminus U$.
...
\begin{enumerate}[(i)]
    ...
    \item Vertex $x$ is not on a fastest temporal paths from $u$ to any $v \in U$.
    Suppose that $x$ lies on a segment $S_{w,z} = (w,z_1,2_3, \dots, z_p, z)$.
    ...
    If $x = w^*$, we have two possibilities, we check the values of each one and set the minimum of them, to equal the $D_{u,x}$ (if possible, if one is to small we immediately reject),
    and again set all other possibilities to the inequality constraint.
    There are $O(k^3)$ different paths from $u$ to $w$ (resp.~$z$), therefore we add $O(k^k)$ inequality constraints for all these paths.   
\end{enumerate} 
