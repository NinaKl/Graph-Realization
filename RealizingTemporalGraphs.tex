...
\usetikzlibrary{shapes,calc,patterns,arrows.meta,snakes}
\tikzset{
...
\begin{claim}
    For all $w\in \hat{W}$ we have that $\lambda(\{w^\star,w\})=t$ for some $t\in\.
\end{claim}
\end{proof}
...
Therefore, we approach the problem using some special properties, which allow us to create $f(k)$ different guesses (where $f$ is a super exponential function in the feedback edge number $k$).
...
We have created $f(k)$ instances (where $f$ is a super exponential function), 
each with $O(k^2)$ variables and $n^2 g(k)$ constraints (again, $g$ is a super exponential function).
