...
\todo[inline]{NK:Following definition and observation are copied from the previous section (need to delete from one part).}
...
\begin{enumerate}
    ...
    \item Find the (partial) labelling $\lambda$ of $G'$, that connects all pairs of vertices from $U$, satisfying their respective values in $D'$.
    \item Extend the partial labelling $\lambda$ to the labeling of the whole $G'$, respecting $D'$.
    \item Extend the labelling $\lambda$ of $G'$ to the labeling of $G$, respecting $D$.
\end{enumerate}
...
Since we determine the labels of edges in $G \setminus G'$ in the last step, the the whole solution is not affected by performing this step.

\paragraph{Step 2.}
To find a minimum feedback edge set $F$ of $G'$ we find a spanning tree $T$ of $G'$ and get $F = E(G) \setminus E(T)$. 
Clearly we perform this step in linear time.
...
We denote with $U = V_1 \cup V_2 \cup V_3$ the set of \emph{vertices of interest}. It follows that $|U| \leq 4k$.
Clearly we perform this step in linear time.

\paragraph{Step 4}
In this step we want to determine the labeling of fastest temporal paths among all vertices in $U$.
For a pair $u,v$ of vertices in $U$, there are $2^{O(k)}$ possible paths in $G'$ between them, where each one could be the fastest temporal path from $u$ to $v$ and $v$ to $u$. Therefore we get $2^{O(k)}$ partial (different) labelings of $G'$. \todo{NK: How??}
This is true for any pair of vertices in $U$. 
We then try to join the labelings of different pairs of vertices and end up with $(2^{O(k)})^{k^2} = 2^{O(k^3)}$ different labelings.
Some of them we can reject immediately, namely if two different labellings label the same edge with the different label.
At the end we end up with $22^{O(k^3)}$ partial labelings $\lambda$ of $G'$, where $(G',\lambda)$ has labels on all fastest paths among any pair of vertices in $U$,  and the duration of the fastest temporal path from $u$, to $v$, where $u,v \in U$ satisfies the value $D_{u,v}$. 
\todo[inline]{Missing details.}
...
\paragraph{Step 6.}
Let $\lambda$ be the labelling of $G'$, that satisfies $D'$.
