...
Let $D$ be an instance of \deltaExact, with $G$ being its underlying graph.
...
\begin{enumerate}
    ...
    \item Compute a minimum feedback edge set $F$ in $G'$.
    \item Determine $O(k)$ vertices of interest $U$ of $G'$, where $k = |F|$.
    \item Find the (partial) labelling of $G'$, that connects all pairs of vertices from $U$, satisfying their respective values in $D'$.
    \item Fill in the labeling for whole $G'$.
    \item Fill in the labeling for $G$, respecting $D$.
\end{enumerate}
