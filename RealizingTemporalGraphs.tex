...
\end{definition}
From the definition it follows, that there are exactly $k^2$ segments in $G'$.
...
%Each of the previous partial ILP instances is therefore extended into $O(k^6)$ new ILP instances.
    ...
    Let 
    $S_{w,z} = (w=z_1,z_2, \dots, z_r = z)$ be another segments in $G'$.
...
\end{equation}
In both cases we implicitly assume that if the difference is smaller than zero we add the value $\Delta$ to it (\ie we consider the difference modulo $\Delta$), therefore we have the sign $\Delta$ around the brackets.

First case we observe is the case when we want to determine the labels on fastest paths from $u$ to $v$, where $u,v \in U$.
