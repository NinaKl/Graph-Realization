...
\end{itemize}
When the queue is empty, all edges have received a label. Iterate over all vertex pairs $u,v$ and check whether the fastest path from $u$ to $v$ in $(G,\lambda)$ has duration $D_{u,v}$. If this check succeeds for all vertex pairs, output the labeling $\lambda$, otherwise abort.
...
%From the construction of $Z$ (\ie by exhaustively removing vertices of degree one from~$G$), it follows that $G[Z]$ (the graph induced in $G$ by $Z$) consists of disjoint trees.  Each of these trees has a unique neighbor $u_i$ in $G'$.  Denote by $T_i$ the tree obtained by considering such a vertex $u_i$ and all the trees from $G[Z]$ that are incident to $u_i$.  We then refer to $u_i$ as the \emph{clip vertex} of the tree $T_i$.


Finally, the set of \emph{important vertices} is defined as the set $U \cup U^{\ast} \cup Z^{\ast}$.
For an illustration see \cref{fig:labelingVertices}. Note that determining sets $U$ and $U^*$ takes linear time.
...
Note from \cref{def:segments} that $S_{u,v} \neq S_{v,u}$ since we consider paths to be directed. It's also worth emphasizing that $S_{v,u}$ is essentially the reversed path of $S_{u,v}$.
...
We now relate also some of these edges.
More precisely,
...
For this, we use the fact that at least one of the temporal paths between vertices in $S_{u,v}$ has to pass through $f$, when $S_{u,v}$ is of length at least $5$ or if it is of length $4$ there is a unique labeling that allows no such path to traverse $f$ (see~\cref{lemma:FPT-noUndeterminedEdgeInSegment}).
%
...
\begin{enumerate}[G-1.]
    ...
    Thus we create $O(1)$ guesses for every such segment $S_{u,v}$, and $O(k^2)$ new guesses in total, as there are at most $O(k^2)$ segments.
    % counter for guesses continues
    \setcounter{guesscounter}{\value{enumi}}
\end{enumerate}
Once we know which temporal paths pass through $f$, 
...
Note, from \cref{lemma:FPT-noUndeterminedEdgeInSegment} it follows that the above procedure holds only for segments of length at least $4$.
...
All of the above is enough to determine the labeling $\lambda$ of $G'$. Now we have to extend the labeling to consider also the vertices of~$Z= V(G) \setminus V(G')$ that we initially removed from $G$.
From the construction of $Z$ (\ie by exhaustively removing vertices of degree one from~$G$), it follows that $G[Z]$ (the graph induced in $G$ by $Z$) consists of disjoint trees. 
Each of these trees has a unique neighbor $u_i$ in $G'$. 
Denote by $T_i$ the tree obtained by considering such a vertex $u_i$ and all the trees from $G[Z]$ that are incident to $u_i$. 
We then refer to $u_i$ as the \emph{clip vertex} of the tree $T_i$.
Recall that $G[Z]$ consists of disjoint trees and that each of these trees has a unique neighbor $u_i$ in $G'$. 

Before we describe how to determine the fastest temporal paths between any vertex from $Z$ and any other vertex in $G$,
we prove the following.

\begin{lemma}\label{lemma:ftp-tree-neighbor}
Let $T_i$ be a tree with $u_i$ as its clip vertex and let $r_i \in V(T_i) \cap N_{T_i}(u_i)$ be an arbitrary vertex of $T_i$ that is a neighbor of $u_i$. 
Now, among all neighbors of $u_i$ in $G'$ we chose the one that is the closest to $r_i$ with respect to the values of $D$. 
In other words, we find $v \in N_{G'}(u_i)$ for which $D_{r_i,v} \leq D_{r_i,w}$ for all $w \in N_{G'}(u_i)$.
Then we have that the path $P^*=(r_i,u_i,v)$ is the fastest temporal path from $r_i$ to $v$.
\end{lemma}
\begin{proof}
Suppose for the contradiction that there exists a path $P^*_2$ from $r_i$ to $v$ that is faster than $P^*=(r_i,u_i,v)$. 
By the structure of $G$ it follows that $P^*_2$ passes through the clip vertex $u_i$ of $T_i$ (as this is the only neighbor of $r_i$ in $G'$), through a vertex $w \in N_{G'}(u_i) \setminus \{v \}$, 
and through some other vertices $u_1, u_2, \dots, u_j$ in $G$ ($j \geq 0$), before it finishes in $v$.
Therefore, $P^*_2= (r_i,u_i,w,u_1,u_2,\dots,u_j,v)$, and $d(P^*_2) \leq d(P^*)$.
...
\end{proof}
Observe that determining a vertex $v$ closest to $r_i$ (with respect to the values of $D$) is a straightforward process. We only need to check the values in the $r_i$-th row of the matrix $D$. Therefore, this process takes linear time.
%
Suppose now that we know that 
$(r_i,u_i,v)$ is the fastest temporal path from $r_i$ in $T_i$ to the vertex $v$.
Then
we can determine the label of the edge $r_i u_i$ with respect to $\lambda(u_i v)$ (see \cref{obs:FirstLabelAndDuration}).\label{lemma:ftp-tree-neighbor}
Furthermore, using the algorithm for trees (see \cref{thm:deltaExact-PolyTimeTrees}), we determine labels on all edges of $T_i$ with respect to the same label.
Using the properties from the trees and similar arguing as in \cref{lemma:ftp-tree-neighbor} we get the following.
\begin{observation}\label{obs:tree-only-neighbors}
Let $T_i$ be a tree described as above, with $u_i$ being its clip vertex. Let $r_i, r_j$ be two vertices in $T_i$ where $r_i$ is a neighbor of $u_i$ and $r_j$ is a neighbor of $r_i$, and let $x \in V(G)\setminus \{r_i,r_j\}$ be an arbitrary vertex in $G$.
Denote with
$P_i$ and $P_j$ the underlying paths of the fastest temporal paths from $r_i$ and $r_j$ to $x$, respectively.
Then $P_i$ and $P_j$ differ only in the edge $r_ir_j$, more specifically $P_j = \{r_ir_j\} \cup P_i$ or $P_i = \{r_i r_j \} \cup P_j$.
\end{observation}
Because of \cref{obs:tree-only-neighbors} in the upcoming analysis we focus only on the vertices of each tree $T_i$ that are neighbors of their corresponding clip vertex $u_i$. 
All fastest temporal paths to and from all other tree vertices are then uniquely extended.
From this point onward, whenever we refer to a vertex $r$ in the tree $T_i$ we actually mean the vertex $r$ that is a neighbor of the clip vertex of the tree $T_i$.

We split our analysis into two cases.
First, when the clip vertex of tree $T_i$ is not a vertex of interest
...
It's worth noting that finding the fastest path between two vertices from the same tree $T_i$ is trivial, so we exclude this case from our analysis.

To efficiently determine the fastest temporal paths to and from the vertices from $Z$ we still need to show some extra properties.
We start with a property for the case when the clip vertex of the tree $T_i$ is not a vertex of interest.
\begin{lemma} \label{lemma:treeLabels-notInteresting}
    Let $z \in V(G')$ be a clip vertex of the tree $T_i$ in $G[Z \cup \{z\}]$, where $z \notin U$. 
    Let $v_1$ and $v_2$ be the two neighbors of $z$ in $G'$.
    Then the labels of the tree $T_i$ can be determined with respect to $\lambda(v_1 z)$ and $\lambda(z v_2)$.
\end{lemma}
%
\begin{proof}
    First observe that since $z$ is not a vertex of interest it must be a part of some segment $S_{u,w}$, where $u,v \in U$ and $z \neq u \neq v$. Therefore, $z$ is of degree $2$ in $G'$.
    Let $r \in V(T_i)$ be a neighbor of $z$.
    We observe the values $D_{r,v_1}, D_{r,v_2}$ and distinguish the following cases.

    First, $D_{r,v_1} = D_{r,v_2}$
    Then, using \cref{lemma:ftp-tree-neighbor} we get that the fastest paths from $r$ to $v_1$ and from $r$ to $v_2$ are of length two.
    Moreover, we know that these two paths consist of the edge $rz$ and one of the edges $zv_1,zv_2$.
    This allows us to determine the label of the edge $rz$ (and consequently all other edges of $T_i$) with respect to $\lambda(zv_1)$ (see \cref{obs:FirstLabelAndDuration}).

    Second, $D_{r,v_1} \neq D_{r,v_2}$. 
    Let us denote with $t_1 = \lambda(z v_1), t_2 = \lambda(z v_2)$ and $t_3 = \lambda(rz)$.
    W.l.o.g. suppose $D_{r,v_1} > D_{r,v_2}$, which implies that $t_1 > t_2$ (the other case is analogous), and suppose that $t_1, t_2 > t_3$.
    Now we claim that $D_{v_1,r} < D_{v_2,r}$.
    Suppose for the contradiction that this is not true. Then $D_{v_2,r} < D_{v_1,r} \leq (\Delta + t_3 - t_1)$. This implies that the fastest temporal path from $v_2$ to $r$ cannot use the path $(v_1,z,r)$, and is therefore of form $(v_2,z,r)$.
    By the definition, the duration of this path is then $D_{v_2,r} = \Delta + t_3 - t_2 + 1$.
    But since $t_1 > t_2$ it follows that $(\Delta + t_3 - t_2) + 1 > (\Delta + t_3 - t_1) + 1$ and by the definition $D_{v_1,r} \leq > (\Delta + t_3 - t_1) + 1$. 
    Therefore we get $D_{v_2,r} > D_{v_1,r} $, a contradiction.
    All together, from $D_{r,v_1} > D_{r,v_2}$ we determine the label of edge $rz$ with respect to $\lambda(z v_2)$,
    and from $D_{v_1,r} < D_{v_2,r}$ we determine the label of edge $rz$ with respect to $\lambda(z v_1)$.
    %
\end{proof}
%
In the case when the clip vertex of the tree $T_i$ is a vertex of interest we cannot determine the labeling of the tree with respect to the labels of all edges incident to the clip vertex.
Therefore, to efficiently determine the fastest temporal paths from and to vertices of $T_i$ in this case we proceed as follows.
We split the vertices of $T_i$ in a small number of equivalence classes (bounded in $k$) and guess the fastest temporal paths among a vertex in each equivalence class and all the vertices from $U \cup U^*$. 
With this, we can determine the fastest temporal between vertices in $Z$ and all other vertices in $G$.
Let us start with describing how to create equivalence classes.

Let $u \in V(G')$ be a clip vertex of the tree $T_i$ in $G[Z \cup \{u\}]$, where $u \in U$. 
We split the vertices of $T_i$ into (at most) $|N_{G'}(u)|$ equivalence classes using the following procedure.
For a vertex $r_i \in V(T_i)$ we determine the vertex $w \in N_{G'}(u)$ that is on the smallest duration from $r_i$.
In other words, we find the vertex $w$ such that for any other $w' \in N_{G'}(u)$ it holds $D_{r_1,w} \leq D_{r_1,w'}$.
We then create an equivalence class $C_w$ and set $r_i \in C_w$.
If there is another $w'' \in N_{G'}(u)$ for which $D_{r_1,w} = D_{r_1,w''}$ then $C_w = C_{w''}$.
We repeat the above procedure for all trees $T_i$ and all their vertices.
In the set $Z^*$ we collect a representative vertex from each of the equivalence classes.
There are $O(k)$ vertices of interest, each with at most $k$ neighbors, therefore we create at most $O(k^2)$ equivalence classes, which implies $|Z^*| \in O(k^2)$.
Note that creating all equivalence classes takes linear time, since for any tree vertex we check the values in the matrix $D$ for the corresponding vertices exactly once.

Let us now show the following important property of equivalence classes.
\begin{lemma}\label{lemma:treeEquivClass-intVertices}
    Let $u \in V(G')$ be a clip vertex of the tree $T_i$ in $G[Z \cup \{u\}]$, where $u \in U$,
    and let $r_i,r_2 \in V(T_i)$ be in the same equivalence class.
    Then, the fastest temporal paths from $r_1$ and $r_2$ to any other vertex in $G$ coincide on the vertices of $V(G) \setminus V(T_i)$.
    Similarly, it holds for the fastest temporal paths from any other vertex in $G$ to $r_1$ and $r_2$.
\end{lemma}
%
\begin{proof}
    Let $y \neq u$ be a vertex in $V(G')$.
    %
    Denote with $P_1$ the underlying path of the fastest temporal path from $r_1$ to $y$, which consists of the edge $r_1 u$ and the path $P$ from $u$ to $y$.
    Similarly, let $Q_2$ be the underlying path of the fastest temporal path from $r_2$ to $y$, consisting of the edge $r_2u$ together with the path $Q$ from $u$ to $y$.
    Define $P_2$ as the second path from $r_2$ to $y$ that first uses the edge $r_1u$ and then the path $P$.
    Similarly, $Q_1$ represents the second path from $r_1$ to $y$ that first uses the edge $r_2u$ and then the path $Q$.
    Our objective is to demonstrate that either $P = Q$ or that $d(P_1, \lambda) = d(Q_1, \lambda)$ (and $d(P_2, \lambda) = d(Q_2, \lambda)$).
    This implies that $r_1$ and $r_2$ use temporal paths that coincide on the vertices of $V(G) \setminus V(T_i)$ to reach $y$.
    
    Let us set the label of the edge $r_1u$ to $t_1$, the label of $r_2u$ to $t_2$, the label of the last edge of the path $P$ as $t_p$ and the label of the last edge of the path $Q$ as $t_q$.
    By the definition, since $P_1$ represents the fastest temporal paths form $r_1$ to $y$ we get that
        $D_{r_1,y} = t_p - t_1 + c_p \Delta$, where $c_p \in \mathbb{N}$.
    Similarly, for the path $Q_2$ it holds that $D_{r_2,y} = t_q - t_2 + c_q \Delta$ with $c_q \in \mathbb{N}$.
    Note that since $r_1$ and $r_2$ are in the same equivalence class, it holds that the first label of $P$ and the first label of $Q$ have to be bigger than $t_1,t_2$. Meaning that the difference between the first label of $P$ (resp.~$Q$) with $t_1$ and $t_2$ is smaller than $\Delta$,
    ...
    Then $c_p \leq c_q + 1$.
    Now, since $r_1$ and $r_2$ are in the same equivalence class and by the definition of the duration of a temporal path we get that $d(P_2, \lambda) = t_p - t_2 + c_p \Delta \leq t_p - t_2 + (c_q - 1)\Delta$. 
    Because $Q_2$ is the fastest path from $r_2$ to $y$ we have also that $d(P_2, \lambda) \geq D_{r_2,y}$, which gives us $t_p - t_2 + (c_q - 1) \Delta \geq t_q - t_2 + c_q \Delta$.
    ...
    Above we proved that $c_p = c_q$. We also know that $d(Q_1,\lambda) \geq d(P_1,\lambda)$ as $P_1$ is the fastest path from $r_1$ to $y$. All of this results in $t_q - t_1 + c_p \Delta \geq t_p - t_1 + c_p \Delta$ implying $t_q \geq t_p$, a contradiction.
    Therefore, $t_p = t_q$.

    We proved that either $P$ and $Q$ are the same, or if they are different then $P_1$ and $Q_1$ are of the same duration and are both fastest paths from $r_1$ to $y$ (the same holds for $r_2$). 

    Proof for the fastest temporal paths in the other direction, namely starting at $y$ and reaching $r_1$ and $r_2$, is done analogously.
\end{proof}
From the above lemma we conclude that determining the fastest temporal path from any vertex within an equivalence class $C_w$ of $T_i$ to a given vertex $y \in V(G)$ requires only knowledge of the fastest temporal path $P^w_y$ from any vertex $r \in C_w$ to $y$. 
For all other vertices in $C_w$ to $y$, the fastest temporal paths share identical vertices and edges as $P^w_y$ outside the tree $T_i$. Within the tree $T_i$, there is a unique way in which the path is extended from these vertices.
The same holds for the temporal paths from $y$ to vertices in $C_w$.
We are now ready to describe our last guess.
\begin{enumerate}[G-1.]
\setcounter{enumi}{\value{guesscounter}}

\item \label{FPT-guessFTPtreesEquivalence}
    We guess the fastest temporal paths between all pairs of vertices of $Z^*$ and $U,U^*,Z^*$.
    Since $Z^*$ is of size $O(k^2)$ and $U \cup U^* \cup Z^*$ is of size $O(k^2)$ 
    we introduce $k^{O(k^5)}$ guesses.
\end{enumerate}
%
Now we are ready to explain how we determine the fastest temporal paths for all the vertices in $Z$

\subparagraph{\boldmath Fastest paths from $z \in Z$ to $y \in U \cup U^*$.}
Let $x \in V(G')$ be a clip vertex of the tree $T_i$ in $G[Z \cup \{x\}]$ with $z \in V(T_i)$.
We distinguish the following two cases.
\begin{enumerate}[(i)]
    \item \label{FPT:clipInU-1}
    The clip vertex $x =u \in U$ is a vertex of interest.
    From guess~\textcolor{lipicsGray}{\textsf{\textbf{G-\ref{FPT-guessFTPtreesEquivalence}}}} we determine the fastest temporal path from $z$ to $y$.
    ...
    Then all the labels of the tree $T_i$ are determined with respect to the label $\lambda(v_{j-1}v_j)$ and with respect to the label $\lambda(v_{j}v_{j+1})$ (see~\cref{lemma:treeLabels-notInteresting}).
    From the calculations above and the performed guesses we know the exact structure (\ie the sequence of vertices and edges) of the following paths:
    \begin{itemize}
    ...
    To be precise, from the guesses performed we know the structure of the fastest path from $v_2$ through $u$ to any other vertex of interest, and any other neighbor of the vertex of interest, which can be easily (uniquely) extended to start from $x=v_i$ 
    (see~guesses \textcolor{lipicsGray}{\textsf{\textbf{G-\ref{FPT-guessFTPamongv2z2}}}} and
    \textcolor{lipicsGray}{\textsf{\textbf{G-\ref{FPT:guess-uToSegmentz2}}}}).
    
    Before we proceed note also that since the labels of the tree are determined with respect to the label of the edge $v_{i-1}v_i$ and $v_{i}v_{i+1}$ we can determine the difference 
    $|\lambda(v_{i-1} x) - \lambda (z x)|$ and $|\lambda(xv_{i+1}) - \lambda (z x)|$.
    %
    Suppose now that $P_{xy}^* = P_{xy}^u$.
    We calculate the value $c = D_{x,y} + |\lambda(v_{i-1} x) - \lambda (z x)|$ and compare it to $D_{z,y}$.
    There are three options.
    First $c = D_{z,y}$, in this case the fastest temporal path from $z$ to $y$ uses first the edge $zx$ and then continues to $y$ using the edges and vertices of $P_{xy}^*$.
    Second $c > D_{z,y}$, in this case the fastest temporal path from $z$ to $y$ uses first the edge $zx$ and then continues to $y$ using the vertices and edges of $P_{xy}^v$.
    Third $c < D_{z,y}$, in this case we stop the calculation and return an empty solution, as it cannot happen that a temporal path has a smaller duration than the corresponding value in the matrix $D$.
%
\end{enumerate}
In both cases, we introduce the equality constraint for the determined fastest temporal path and inequality constraint for all the other $k^{O(k)}$ paths.

\subparagraph{\boldmath Fastest paths from $z \in Z$ to $y \in V(G) \setminus (U \cup U^*)$.}
The proof in this case is similar to the one above.  We still split the analysis into two parts, one where the clip vertex $x$ of a tree $T_i$ that includes $z$ is in $U$ and one where it is not in $U$.
The difference is that in some cases we need to also extend the ending part of the path (which can be done uniquely, using the same arguments as in the previous case).

Once we determine the fastest temporal path from $z$ to $y$ we introduce an equality constraint for it, and for all other $k^{O(k)}$ paths we introduce the inequality constraints.
