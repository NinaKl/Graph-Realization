...
At the end we create $|N_{G'}(v)|=O(k)$ sets $C_w$, whose union contains all children of $v$ in $T_v$. 
For every two sets $C_w$ and $C_{w'}$, where $w,w'\in N_{G'}(v)$, we have that either $C_w = C_{w'}$, or $C_w \cap C_{w'} = \emptyset$. 
We call each of these sets $\{C_w : w \in N_{G'}(v)\}$ an \emph{equivalence class} of the tree $T_v$. 
Now, from each equivalence class $C_w$ we choose an arbitrary vertex $r_w$ and put it into the set $Z^*_v$.
%
...
\end{equation}
%
Since $|U| \in O(k)$ and for each $u \in U$ it holds $|N_{G'}(u)| = O(k)$, we get that $|Z^*| \in O(k^2)$. 
...
Remember, in the case where the clip vertex $u$ of the tree $T_u$ in $G[Z \cup \{u\}]$ is a vertex of interest, we split the vertices in the first layer of $T_u$ into at most $|N_{G'}(u)|$ equivalence classes.
...
\begin{lemma}\label{lemma:treeEquivClass-intVertices}
    ...
    and let $z_1,z_2 \in V(T_v)$ be in the same equivalence class of the tree $T_u$.
    Then, the fastest temporal paths from $z_1$ and from $z_2$ to any other vertex in $G'$ coincide on the edges of $G'$.
    Similarly, the fastest temporal paths from any other vertex in $G'$ to $z_1$ and to $z_2$ coincide on the edges of $G'$.
\end{lemma}
%
\begin{proof}
    ...
    Define $P_2$ as the second path from $z_2$ to $y$ that first uses the edge $z_1u$ and then the path $P$.
    Similarly, $Q_1$ represents the second path from $z_1$ to $y$ that first uses the edge $z_2u$ and then the path $Q$.
    Our objective is to demonstrate that either $P = Q$ or that $d(P_1, \lambda) = d(Q_1, \lambda)$ (and $d(P_2, \lambda) = d(Q_2, \lambda)$).
    This implies that $z_1$ and $z_2$ use temporal paths that coincide on the vertices of $V(G) \setminus V(T_u)$ to reach $y$.
    ...
    Note that since $z_1$ and $z_2$ are in the same equivalence class, it holds that the first label of $P$ and the first label of $Q$ have to be bigger than $t_1,t_2$. Meaning, that the difference between the first label of $P$ (resp.~$Q$) with $t_1$ and $t_2$ is smaller than $\Delta$,
...
\begin{observation}\label{obs:FPT-4determined}
    Let $S_{u,v}$ be an arbitrary segment in $G'$.
    If $S_{u,v}$ is of length at least $4$ then the labels of all the edges are determined with respect to its first edge.
...
Determining the fastest temporal paths between any two vertices in the same tree is a straightforward process (see~\ref{thm:deltaExact-PolyTimeTrees}), so we exclude this case from our upcoming analysis.
From \cref{obs:tree-only-neighbors} it follows that knowing temporal paths between any $y  \in V(G') \setminus (T_v)$ and all vertices in the first layer of the tree $T_v$ (\ie children of the root $v$),
...
Let $x \in V(G')$ be a clip vertex of the tree $T_x$ in $G[Z \cup \{x\}]$ with $r \in V(T_i)$ be a vertex in the first layer of $T_x$.
...
\begin{enumerate}[(i)]
    ...
    Then all the labels of the tree $T_x$ are determined with respect to the label $\lambda(v_{j-1}v_j)$ and with respect to the label $\lambda(v_{j}v_{j+1})$ (see~\cref{lemma:treeLabels-notInteresting}).
    ...
    Before we proceed note also that the labels of the tree are determined with respect to the label of the edge $v_{i-1}v_i$ and $v_{i}v_{i+1}$ (see~\cref{lemma:treeLabels-notInteresting}), therefore we can determine the difference 
    $|\lambda(v_{i-1} x) - \lambda (r x)|$ and $|\lambda(x v_{i+1}) - \lambda (r x)|$.
    %
    Suppose now that $P_{xy}^* = P_{xy}^u$.
    We calculate the value $c = D_{x,y} + |\lambda(v_{i-1} x) - \lambda (r x)|$ and compare it to $D_{r,y}$.
...
Once we determine the fastest temporal path from $r$ to $y$ we introduce an equality constraint for it, and for all other $k^{O(k)}$ paths we introduce the inequality constraints.
