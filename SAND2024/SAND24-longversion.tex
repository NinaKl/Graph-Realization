...
From the construction of $Z$ (\ie by exhaustively removing vertices of degree one from~$G$), it follows that $Z$ consists of disjoint trees. 
Each of these trees has a unique neighbor $u_i$ in $G'$. Denote by $T_i$ the tree that is obtained by such a vertex $u_i$ of $G'$ and all the trees of $Z$ that are incident to $u_i$. Then  we call $u_i$ the \emph{clip vertex} of the tree $T_i$. 
...
In the case when segment has $4$ edges, it can happen that the fastest temporal paths from above, do not traverse $f$. But in this case we know that three edges have the same label, denote it $c$, while the label of the last one is a function of $c$.
