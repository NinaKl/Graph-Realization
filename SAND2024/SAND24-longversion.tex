...
From the construction of $Z$ (\ie by exhaustively removing vertices of degree one from~$G$), it follows that $G[Z]$ (graph induced in $G$ by $Z$) consists of disjoint trees. 
Each of these trees has a unique neighbor $u_i$ in $G'$. Denote by $T_i$ the tree that is obtained by such a vertex $u_i$ and all the trees from $G[Z]$ that are incident to $u_i$. Then  we call $u_i$ the \emph{clip vertex} of the tree $T_i$. 
For each of these trees~$T_{i}$, we select an arbitrary vertex $r_i \in Z$ that is a neighbor of the clip vertex $u_i$ in $T_i$,
and call it the \emph{representative vertex} of the tree $T_{u_i}$. 
We now define by $Z^*$ the set of all representatives $r_i$ of the trees $T_i$,
where the clip vertex $u_i$ of $T_i$ is a vertex of interest, \ie
\[
Z^* = \{ r_i  :  r_i \in Z \text{ is a neighbor of the clip vertex $u_i$ in the tree $T_i$, and $u_i \in U$}\}.
\]
...
Let $S_{u,v}$ be a segment in $G$ of length at least $5$, denoted as $S_{u,v}= (u=v_1,v_2, \dots, v_p = v)$, where $p > 5$.
It is impossible for an inner edge $f = v_i v_{i+1}$ from $S_{u,v} \setminus \{u,v\}$ (where $f$ is an edge that is not incident to a vertex from $U$)
...
In the case where $p = 5$ all temporal paths of length $2$ avoid $f$ if and only if $f$ has the same label as both of the edges incident to it, while the label of the last remaining edge is a function of $\lambda(f)$.
\end{lemma}
...
\begin{proof}
    ...
    %
    Note now that the fastest temporal path from $v_2$ to $v_4$ must first use the edge $u v_2$ and finish with the edge $v_4 v_5$, and it has to be of duration $D_{v_2, v_4}$. Using \cref{lemma:FPT-uv-Labelalledges} we determine the label of the edge $v_4v_5$ as a function of $\lambda(f)$.
\end{proof}
...
\begin{enumerate}[G-1.]
    ...
    Thus we create $4$ guesses for every such segment $S_{u,v}$, and $O(k^2)$ new guesses in total, as there are at most $O(k^2)$ segments.
    % counter for guesses continues
    \setcounter{guesscounter}{\value{enumi}}
\end{enumerate}
...
In the case when a segment has $4$ edges, it can happen that the fastest temporal paths from above, do not traverse $f$. But in this case, we know that three edges have the same label, denote it $c$, while the label of the last one is a function of $c$.
...
Namely, for an edge $e$ that is not yet determined, we set its value to $x_e$ and check the labels of all of its neighbors,
...
All of the above is enough to determine the labeling $\lambda$ of $G'$. Now we have to make sure that the labeling considers also the vertices of~$Z= V(G) \setminus V(G')$ that we initially removed from $G$.
Recall that $Z$ consists of disjoint trees, and that each of these trees has a unique neighbor $u_i$ in $G'$. 
Furthermore, recall that $T_i$ denotes the tree that is obtained by such a vertex $u_i$ of $G'$ and all the trees of $Z$ that are incident to $u_i$; then $u_i$ is the \emph{clip vertex} of the tree $T_i$. 
Moreover, for each of these trees~$T_{i}$ we have selected an arbitrary vertex $r_i \in Z$ that is a neighbor of the clip vertex $u_i$ in $T_i$, 
and $r_i$ is called the \emph{representative vertex} of the tree $T_{u_i}$. 
Let us now describe now how to determine the labels on the edges of any such tree $T_i$.
To determine the correct label of all edges of $T_i$ we use the following property.
\begin{lemma}
Let $T_i$ be a tree having $u_i$ as its clip vertex, and let $e_i = (u_i,r_i)$. 
Among all neighbors of $u_i$ in $G'$, let $v \in N_{G'}(u_i)$ be the one that is closest vertex to $r_i$ (with respect to the values of $D$), \ie  $D_{r_i,v} \leq D_{r_i,w}$ for all $w \in N_{G'}(u_i)$.
Then the path $P^*=(r_i,u_i,v)$ is the fastest temporal path from $r_i$ to $v$.
\end{lemma}
\begin{proof}
...
we present the study for cases when we find the fastest temporal paths from and to the 
representative vertex $r_i$ of tree $T_i$.
Each of these paths is then uniquely extended to all vertices in $T_i$.
...
\begin{enumerate}[(i)]
    ...
    Let $x = z_i$ be a part of a segment $S_{w,z} = (w = z_1, z_2, \dots, z_r = z)$, and denote with $z_{i-1}$ and $z_{i+1}$ the neighboring vertices of $x$, 
