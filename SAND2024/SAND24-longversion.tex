...
In the case $v$ is a vertex of interest we define also the set $Z^*_v$ of \emph{representative vertices} of $T_v$, as follows.

\noindent
We first create an empty set $C_w$ for every vertex $w$ that is a neighbor of $u$ in $G'$.
We then iterate through every vertex $r$ that is in the first layer of the tree $T_v$ (\ie vertex that is a direct child from the root $v$), we check the matrix $D$ and find the vertex $w \in N_{G'}(v)$ that is on the shortest duration from $r$.
In other words, for a $r \in N_{T_v}(v)$ we find $w \in N_{G'}(v)$ such that $D_{r,w} \leq D_{r,w'}$ for all $w' \in  N_{G'}(v)$.
%
We add vertex $r$ to $C_w$.
In the case when there exist another $w'' \in  N_{G'}(v) $ for which $D_{r,w''} = D_{r,w}$, we add $r$ also to the set $C_{w''}$. In fact, in this case $C_{w''} = C_w$.
At the end we create $|N_{G'}(v)|$ sets $C_w$, containing all vertices of the first layer of $T_v$.
We call these sets \emph{equivalence classes} of the tree $T_v$.
Now, from each equivalence class $C_w$ we chose an arbitrary vertex $r_w$ and put it into the set $Z^*_v$.
%
We repeat the above procedure for all trees $T_u$ with the clip vertex $u$ from $U$, and define $Z^*$ as 
$$Z^* = \bigcup\limits_{u \in U} Z^*_u.$$
%
Since $|U| \in O(k)$ and for each $u$ it holds $|N(u)| \in O(k)$, we get that $|Z^*| \in O(k^2)$. 
...
We start by guessing the following.
\begin{enumerate}[G-1.]
...
We now present some properties that involve the vertices from $Z$, that form the trees in $G[Z]$.
\begin{lemma}\label{lemma:ftp-tree-neighbor}
Let $v \in V(G')$ be a clip vertex of the tree $T_v$ in $G[Z \cup \{v\}]$, and let $r_i \in N_{T_v}(v)$ be an arbitrary vertex of $T_v$ that is a neighbor (child) of $v$. 
Among all neighbors of $v$ in $G'$ let $w$ be the one that is on the smallest duration away from $r_i$ with respect to the values of $D$. 
In other words, $w \in N_{G'}(v)$ such that $D_{r_i,w} \leq D_{r_i,w'}$ for all $w' \in N_{G'}(v)$.
Then, the path $P^*=(r_i,v,w)$ represents the fastest temporal path from $r_i$ to $w$.
%
Moreover, all labels of the tree $T_v$ are determined with respect to the label $\lambda(vw)$.
\end{lemma}
\begin{proof}
Suppose for contradiction that there exists a path $P^*_2$ from $r_i$ to $w$ that is faster than $P^*=(r_i,v,w)$. 
By the structure of $G$, it follows that $P^*_2$ passes through the clip vertex $v$ of $T_v$ (as this is the only neighbor of $r_i$ in $G'$), continues through a vertex $w' \in N_{G'}(v) \setminus \{w \}$, 
and through some other vertices $u_1, u_2, \dots, u_j$ in $G$ ($j \geq 0$) before finishing in $w$.
Therefore, $P^*_2= (r_i,v,w',u_1,u_2,\dots,u_j,w)$, and $d(P^*_2,\lambda) \leq d(P^*,\lambda)$.
Now, since $D_{r_i,w} \leq D_{r_i,w'}$ for all $ w' \in N_{G'}(v)$ the first part of path $P^*_2$ from $r_i$ to $w'$ takes at least $D_{r_i,w}$ time.
Since $w \neq w'$, we need at least one more time-step (one more edge) to traverse from $w'$ to reach $w$. 
Therefore, $d(P^*_2, \lambda) \geq D_{r_i,w} + 1$ which implies that $P^*$ is a faster temporal path than $P^*_2$.

For the second part, knowing that the duration of $P^*$ is $D_{r_i,w}$ we can determine the label of the edge $r_i v$ with respect to the label $\lambda(vw)$ (see \cref{obs:FirstLabelAndDuration}).
Furthermore, using the algorithm for trees (see \cref{thm:deltaExact-PolyTimeTrees}), we can now determine labels on all edges of $T_v$ with respect to the same label $\lambda(v w)$.
\end{proof}
...
%%%% TODO NKL some description of equivalence classes
...
\begin{lemma}\label{lemma:treeEquivClass-intVertices}
    ...
    and let $r_i,r_2 \in V(T_i)$ be in the same equivalence class of the tree $T_i$.
...
$r_i \in N_{T_v}(v)$ be a child of $v$ in the tree $T_v$, and let $r_j$ be a descendant of $r_i$ in the tree $T_v$.
...
Define $Q$ as the (unique) path between $r_i$ and $r_j$ in $T_v$.
Let $x \in V(G)\setminus V(T_v)$ be an arbitrary vertex in $G$.
Denote with
$P_i$ and $P_j$ the underlying paths of the fastest temporal paths from $r_i$ and $r_j$ to $x$, respectively.
Then $P_i$ and $P_j$ differ only in the edges and vertices of $Q$.
\end{observation}
The correctness from the above observation is a consequence of \cref{lemma:treeEquivClass-intVertices} and the fact that $P_i$ and $P_j$ leave the tree $T_i$ using the same edge $r_ix$.
...
From \cref{lemma:treeEquivClass-intVertices}, we conclude that determining the fastest temporal path from any vertex within an equivalence class $C_w$ of $T_i$ to a given vertex $y \in V(G)$ requires only knowledge of the fastest temporal path $P^w_y$ from any vertex $r \in C_w$ to $y$. 
