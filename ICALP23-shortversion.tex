...
We complement this result by showing that the problem is fixed-parameter tractable (FPT) with respect to the feedback edge number of the underlying graph (\cref{thm:FPTwrtFES}). 
Our FPT algorithm preprocesses the input graph in a suitable way that allows us to efficiently guess desired structures. Then we create integer linear programs (ILPs) (with bounded number of variables) and add constraints to those instances by suitably partitioning the fastest paths among pairs of vertices belonging to a small number of interesting sets. The notion of segment is crucial for adding the necessary and sufficient number of linear constraints. We use Lenstra's Theorem~\cite{Lenstra1983Integer} to solve the ILPs efficiently.


Note that this provides a fairly complete picture of the parameterized computational complexity of \deltaExact\ with respect to structural parameters of the underlying graph that measure ``tree-likeness''. To obtain our results, we prove several properties of fastest temporal paths, which may be of independent interest.
...
To the best of our knowledge, we are the first to consider graph realization problems in the temporal setting. However, many other connectivity-related problems have been studied in the temporal setting~\cite{enright2021deleting,MolterRZ21,klobas2023interference,deligkas2022optimizing,erlebach2021temporal,Flu+19a,Zsc+19,CasteigtsCS22}, most of which are much more complex and computationally harder than their non-temporal counterparts, and some of which do not even have a non-temporal counterpart.
...
\centering
%\begin{center}
\begin{tikzpicture}[line width=1pt, scale=.65, yscale=.6]
\scriptsize
\node (A) at (14,0) {};
...
\node[rotate=-90] (A) at (7,-21.5) {verification gadget for $j$};
	...
	\draw[edge, line width=1.5pt] (X1) --node[] {} (VS);
...
Besides that we also need to make sure that all temporal paths that do not follow our guessed paths are not too fast. 
 ...
 For us this means that even if we guess a fastest path between two vertices, we cannot simply determine all labels of the edges used by the fastest path.
	...
	\begin{figure}[t]
    \centering
	\begin{tikzpicture}[xscale=2,yscale=0.6]
	%%%S_uv
