...
are the vertex degrees~\cite{cai2000solution,rfgh611961hu,hakimi1962realizability,barNoy2019graphRealizations,barNoy2021selected,erdos1960graphs}, distances~\cite{barNoy2022GraphRealization,barNoy2021composed,hakimi1965distance}, eccentricities~\cite{barNoy2020efficiently,hell2009linear,behzad1976eccentric,lesniak1975eccentric}, connectivity~\cite{fulkerson1960zero,frank1992augmenting,chen1966realization,frank1994connectivity,frank1970connectivity,gomory1961multi}, etc. 
The motivation for network realization problems stems from both ``verification'' applications where we attempt to reconstruct an existing network of unknown structure based on the outcomes of experimental measurements, and engineering applications related to network design~\cite{augustine2022distributed,grotschel1995design}. 
...
Sections for ICALP:
\begin{enumerate}
    \item Intro 2 PAGES (GEORGE + PAUL)
    \item Prelims 1 PAGE
    \item Hardness (HENDRIK SHORTENS)
    \begin{enumerate}
        \item NP 1/2 PAGE (APPENDIX)
        \item W1 (3 PAGES)
    \end{enumerate}
    \item Algos (NINA SHORTENS)
    \begin{enumerate}
        \item Tree ALGO 2 PAGE
        \item FPT ALGO (6 PAGES)
        \item Cylce ALGO (APPENDIX)
    \end{enumerate}
    \item Conclusion (?) 1/2 PAGE
\end{enumerate}
...
\section{Motivation}
    ...
    This also explains why our FPT algorithm, w.r.t the feedback edge number, is so complex. Usually, the feedback edge number parameter (denote it $k$) provides an easy algorithm, 
...
$t_i\in \lambda_\Delta(u_{i-1}u_i)$ for every $i=1,\ldots,k$, and $t_1<t_2<\ldots<t_k$. 
...
where we actually mean that $(P,\lambda)$ is a temporal path with the underlying path $P$.

In their work Wu et~al.~\cite{Wu2016Efficient} provide an algorithm that computes the duration of the fastest temporal path from a source vertex $v$, in a temporal graph $(G,\lambda)$, to all other vertices,
that runs in polynomial-time.
Using their algorithm, given a temporal graph $(G,\lambda)$, one can determine the whole matrix $D$ in polynomial-time,
where $D$ consists of durations of fastest temporal paths among all pairs of vertices in $(G,\lambda)$.
...
In this section 
we show that in the case when the underlying graph $G$ of an instance $(D, \Delta)$ of \deltaExact\ is a tree or a cycle,
we can determine desired $\Delta$-periodic labeling $\lambda$ of $G$ in polynomial time.
We first start with defining certain notions, that will be of use when solving the problem.

    \begin{definition}[Travel delays]
    Let $(G, \lambda, \Delta)$ be a temporal graph satisfying conditions of \deltaExact.
...
By the definition, each vertex is on distance, and therefore also duration $0$ from itself. This corresponds with all diagonal entries of $D$ being $0$.
...
\subsection{\deltaExact\ is NP-hard}
In this section we prove that it is NP-hard to determine the $\Delta$-periodic temporal graph $(G,\lambda)$,
for a general instance $(D, \Delta)$ of \deltaExact\,
where $\Delta \geq 3$.

\begin{theorem}
	\deltaExactLong\ is \NP-hard for a general $(D,\Delta)$, where $\Delta \geq 3$.
\end{theorem}
...
\end{figure}
%
Now we specify the durations in the matrix $D^\phi$ between all vertex pairs.
...
\end{itemize}
Lastly, observe that $\Delta$ has to be greater or equal to $3$, as the above constructed labeling $\lambda$ uses values from $\{1,2,3\}$.
\end{proof}


\subsection{W[1]-hardness}
...
All durations between non-adjacent vertex pairs that are not specified above are set to infinity.
...
\end{tikzpicture}
    ...
    \caption{Illustration of part of the underlying graph $G$ and a possible labelling. Edges incident with vertices of connector gadgets are omitted. Gray vertices form a feedback vertex set.
    ...
    consist of $5n$ vertices $a_1^{i,j,3},a_2^{i,j,3},\dots,a_{5n}^{i,j,3}$ 
    and $b_1^{i,j,3},b_2^{i,j,3},\dots,b_{5n}^{i,j,3}$, respectively.
    }\label{fig:hardness1}
\end{figure}
...
The last vertex of the edge selection gadget visited by the path segment is (before traversing the vertices in $\{b^{j+1,i',\ell'}_{\ell''''}\mid \ell''''\in[5n]\}$) some vertex $u^{\ell'}_{\ell'''}$ with $\ell'''\in\{0,1,\ldots,4n\}$. By construction of $G$, the duration of a fastest path between $u^\ell_{\ell''}$ and $u^{\ell'}_{\ell'''}$ (in $G$) is at least $3n$.
...
\section{\deltaExact\ is FPT}
In this section we present an FPT algorithm for \deltaExact\ parameterized it by the feedback edge number of the underlying graph.
...
We approach the problem using some special properties, which allow us to create $f(k)$ different guesses (where $f$ will be a double exponential function in the feedback edge number $k$).
We solve each guess by creating an ILP instance with $O(k^3)$ variables and $O(n^c)$ constraints, for some $c \in \mathbb{N}$.
If the ILP instance is a \textsc{Yes} instance then we can will show that also $D$ is a \textsc{Yes} instance of \deltaExact.
...
We now introduce the algorithm, that determines a labeling of $G$ satisfying $D$, or determines that no such labeling exists, and that runs in $f(k) \cdot \text{poly}(n)$ time.
...
\[U^* = \{v \in V(G') \setminus U \mid u \in U, v \in N(u)\}.\]
...
Observe that a temporal path in $G'$ between two vertices of interest is either a segment, or consists of a sequence of some segments. Furthermore, since we have at most $4k$ interesting vertices in $G'$, we can deduce the following.
...
We start with an empty labeling of edges and start specifying the labels of edges.
...
%\todo[inline]{HM: the last sentence in \cref{lemma:FPT-uv-LabelAlmostalledges} is strange. it doesn't say anything. either give the specific restrictions or remove it.}
...
\todo[inline]{Check all of the above and correct it   HM: i checked it and didnt find any issues.}
...
\todo[inline]{HM: i think we should elaborate a little bit more on correctness. basically saying that because we do exhaustive search, we are not overlooking solutions. and because we add all constraints, we can be sure that if the ILP is a yes-instance, there is actually a labeling.}
...
we use this solution and produce our labelling $\lambda$, for which $(G,\lambda)$ realizes the matrix $D$.
This is true since each ILP instance corresponds to a specific configuration of fastest temporal paths in the graph (\ie considering all ILP instances is equivalent to exhaustively searching through all possible temporal paths among vertices).
...
But if there is an instance with a positive solution, then this instance gives rise to the desired labelling, as it satisfies all of the constraints.

To create the labelling $\lambda$ from a solution $X$, of a positive ILP instance,
