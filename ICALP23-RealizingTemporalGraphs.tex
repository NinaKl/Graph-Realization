...
We give a reduction to a variant of \deltaExactLong\ where the period $\Delta$ is infinite (that is, there are no periods) and we allow $D$ to have infinity entries, meaning that the two respective vertices are not temporally connected. At the end of the proof we show how to obtain the result for finite $\Delta$ and a distance matrix $D$ that only has integer entries.
    ...
    \begin{itemize}
        ...
        \item We have a set $\hat{U}_i$ of $11n$ vertices $\hat{u}^i_1,\hat{u}^i_2,\ldots,\hat{u}^i_{11n}$.
    \end{itemize}
    ...
    We add edge $\{v^i_{i-1},\hat{u}^i_1\}$ and for all $\ell\in[11n-1]$ we add edge $\{\hat{u}^i_\ell,\hat{u}^i_{\ell+1}\}$. Furthermore, we add edge $\{\hat{u}^i_{11n},v^i_i\}$. 
...
%Furthermore, we add edge $\{v_{i+1}^i,b^{i,i+1,\ell}_{1}\}$ and edge $\{b^{i,i+1,\ell}_{5n},u^\ell_{\ell'}\}$ between $b^{i,i+1,\ell}_{5n}$ and the vertex $u^\ell_{\ell'}$ of the edge selection gadget of color combination $i,i+1$.
    ...
    Let $i<j\le k$ (skip if $i=k$), let $e_\ell^{i,j}\in F_{i,j}$, and let $w^i_{\ell'}\in W_i$ be incident with $e_\ell^{i,j}$. Then we add edge $\{v_{j-1}^i,a^{i,j,\ell}_{1}\}$ and edge $\{a^{i,j,\ell}_{5n},u^\ell_{n+\ell'-1}\}$ between $a^{i,j,\ell}_{5n}$ and the vertex $u^\ell_{n+\ell'-1}$ of the edge selection gadget of color combination $i,j$.
...
\begin{itemize}
    \item We set $d(v^i_j,v^i_{j'})=(20n+2)(j'-j)+2n-1$.
\end{itemize}
%For all 
%$\ell\in[n+3]$ and 
...
\begin{claim}\label{claim:8}
    ...
    For $i=1$ we have that $\lambda(\{v_0^i,\hat{u}^i_1\})\le n^6$ or $\lambda(\{v_0^i,\hat{u}^i_1\})\ge n^7+2$.
\end{claim}
\begin{claimproof}
...
\begin{claim}\label{claim:9}
    ...
    For $i=k$ we have that $\lambda(\{v_{k}^i,\hat{u}^i_{11n}\})\le n^7+2$.
%
...
Consider the verification gadget for color $i$. Recall that $d(v_0^i,v_k^i)=k(20n+2)+2n-1$. Let $P$ be a fastest temporal path from $v_0^i$ to $v_k^i$.
...
\end{claim}
\begin{claimproof}
Assume for contradiction that $P$ visits a vertex of a connector gadget. Then by \cref{claim:2} we have that the arrival time of $P$ is at least $n^7$. By \cref{claim:2} and \cref{claim:9} we have that the arrival time of $P$ is at most $n^7+2$. This means that the second vertex visited by $P$ cannot be a vertex from a connector gadget, because by \cref{claim:2} this would imply $d(v_0^i,v_k^i)\le 2$. Now we can deduce with \cref{claim:8} that $P$ must have a starting time of at most $n^6$. It follows that the arrival time of $P$ must be smaller than $n^7$, a contradiction to the assumption that $P$ visits a vertex of a connector gadget. 
\end{claimproof}
...
\begin{claimproof}
    ...
    We can conclude that $v^i_j$ can reach a vertex $u^\ell_{\ell'}$ of the edge selection gadget for $i,j+1$ (or $j+1,i$) and a vertex $u^{\ell'}_{\ell'''}$ of the edge selection gadget for color combination $i,j$ (or $j,i$), each along paths of length at least $5n$. From $u^\ell_{\ell'}$ and $u^{\ell'}_{\ell'''}$ we have that any other vertex of the edge selection gadget for $i,j+1$ (or $j+1,i$) and the edge selection gadget for color combination $i,j$ (or $j,i$), respectively, can be reached by a path of length at most $n+3$. Together with the observation made in the beginning of the proof, we can conclude that $v^i_j$ can potentially reach any vertex in $\{v^i_{j-1},v^i_j,v^i_{j+1},v^j_{i-1},v^j_i,v^{j+1}_{i-1},v^{j+1}_i\}$ by a path that visits at most $11n+3$ vertices. 
    
    Lastly, consider the case that $j=i-1$ or $j=i$. Then we have that $v^i_{i-1}$ and $v^i_i$ are connected via a path inside the verification gadget for color $i$, visiting the $11n$ vertices in $\{\hat{u}^i_\ell\mid \ell\in[11n]\}$. The claim follows.
\end{claimproof}
...
\end{claim}
\begin{claimproof}
    As argued in the proof of \cref{claim:11}, a temporal path $P$ from $v^i_j$ to a vertex in $\{v^i_{j-1},v^i_j,v^i_{j+1},v^j_{i-1},v^j_i,v^{j+1}_{i-1},v^{j+1}_i\}$ has to either traverse two segments of $5n$ vertices in $\{a^{i',j',\ell}_{\ell'}\mid \ell'\in[5n]\}$ or $\{b^{i',j',\ell}_{\ell'}\mid \ell'\in[5n]\}$ for some $\ell\in[m]$ and $i',j'\in\{i-1,i,j,j+1\}$ or a segment of the $11n$ vertices in $\{\hat{u}^i_\ell\mid \ell\in[11n]\}$. We analyse the former case first.
    ...
    In the latter case, where $P$ traverses a segment of the $11n$ vertices in $\{\hat{u}^i_\ell\mid \ell\in[11n]\}$, we can make an analogous argument, since all vertices in $\{\hat{u}^i_\ell\mid \ell\in[11n]\}$ are contained in the set $A$ of the second connector gadget of the verification gadget of color $i$ but not in the set $B$ of that connector gadget.
\end{claimproof}

Recall that $P$ denotes a fastest temporal path from $v_0^i$ to $v_k^i$ and that $d(v_0^i,v_k^i)=k(20n+2)+2n-1$. By \cref{claim:10,claim:11,claim:12} he have that $P$ needs to visit at least one vertex in $\{v^{i'}_{j'}\mid i',j'\in \{0,1,\ldots,k\}\}\setminus\{v^i_0,v^i_k\}$. Next, we analyse which vertices in this set are visited by $P$.
...
Now we have by \cref{claim:12,claim:14} that we can divide $P$ into $k$ segments, the subpaths from $v^i_{j-1}$ to $v^i_j$ for $j\in [k]$. We show that all subpaths except the one from $v_{i-1}^i$ to $v_i^i$ have duration $20n+1$. The subpath from $v_{i-1}^i$ to $v_i^i$ has duration $22n+1$. To do.

\subparagraph{$(\Leftarrow)$:} Assume $H$ is a yes-instance of \textsc{Multicolored Clique} and let $X$ be a solution. To do.

\subparagraph{Infinity Gadget.} Finally, we show how to get rid of the infinity entries in $D$ and how to allow a finite $\Delta$. To this end, we introduce the \emph{infinity gadget}. To do.
\end{proof}
