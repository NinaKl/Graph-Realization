...
are the vertex degrees~\cite{cai2000solution,rfgh611961hu,hakimi1962realizability,barNoy2019graphRealizations,barNoy2021selected}, distances~\cite{barNoy2022GraphRealization,barNoy2021composed,hakimi1965distance}, eccentricities~\cite{barNoy2020efficiently,hell2009linear}, connectivity~\cite{fulkerson1960zero,frank1992augmenting,chen1966realization,erdos1960graphs}, etc. 
The motivation for network realization problems stems from both ``verification'' applications where we attempt to reconstruct an existing network of unknown structure based on the outcomes of experimental measurements, and engineering applications related to network design~\cite{augustine2022distributed}. 
...
When it is clear from the context, we drop $\Delta$ form the notation and say that $(G,\lambda)$ is a $\Delta$-periodic temporal graph.
...
\section{Motivation}
	...
	which states that if a shortest path $P$ from vertex $u$ to $v$ traverses the vertex $w$,
...
\begin{claim}\label{claim:12}
    Let $i,j\in\{0,1,\ldots,k\}$. Let $P$ be a temporal path starting at $v^i_j$ and visiting at most $13n+1$ vertices and no vertex of a connector gadget or the alignment gadget. Then $P$ cannot visit vertices in $\{v^{i'}_{j'}\mid i',j'\in \{0,1,\ldots,k\}\}\setminus\{v^i_{j-1},v^i_j,v^i_{j+1},v^j_{i-1},v^j_i,v^{j+1}_{i-1},v^{j+1}_i\}$.
\end{claim}
\begin{claimproof}
    ...
    Lastly, consider the case that $j=i-1$ or $j=i$. Then we have that $v^i_{i-1}$ and $v^i_i$ are connected via a path inside the verification gadget for color $i$, visiting the $13n+1$ vertices in $\{\hat{u}^i_\ell\mid \ell\in[13n+1]\}$. The claim follows.
\end{claimproof}
...
\begin{claimproof}
    ...
    In the latter case, where $P$ traverses a segment of the $13n+1$ vertices in $\{\hat{u}^i_\ell\mid \ell\in[13n+1]\}$, we can make an analogous argument, since all vertices in $\{\hat{u}^i_\ell\mid \ell\in[13n+1]\}$ are contained in the set $A$ of the second connector gadget of the verification gadget of color $i$ but not in the set $B$ of that connector gadget.
\end{claimproof}

Recall that $P$ denotes a fastest temporal path from $v_0^i$ to $v_k^i$ and that $d(v_0^i,v_k^i)=k(20n+6)+6n-1$. By \cref{claim:10,claim:11,claim:12} he have that $P$ needs to visit at least one vertex in $\{v^{i'}_{j'}\mid i',j'\in \{0,1,\ldots,k\}\}\setminus\{v^i_0,v^i_k\}$. Next, we analyse which vertices in this set are visited by $P$.
...
Now we have by \cref{claim:12,claim:14} that we can divide $P$ into $k$ segments, the subpaths from $v^i_{j-1}$ to $v^i_j$ for $j\in [k]$. We show that all subpaths except the one from $v_{i-1}^i$ to $v_i^i$ have duration $20n+5$. The subpath from $v_{i-1}^i$ to $v_i^i$ has duration $26n+5$. To do.
...
\subparagraph{$(\Leftarrow)$:} Assume $H$ is a yes-instance of \textsc{Multicolored Clique} and let $X$ be a solution. We construct the following labelling for the underlying graph $G$, see also \cref{fig:hardness1} for an illustration.
...
Recall that the following distance requirements were specified in the construction of the instance. It is straightforward to verify that distance requirements we recall in the following are all met, assuming no faster connections are introduced.
\begin{itemize}
...
\end{itemize}
For two connector gadgets, one with vertices $\hat{v}_0,\hat{v}_0',\hat{v}_1,\hat{v}_2,\hat{v}_3,\hat{v}_3'$ and with sets $A$ and $B$, and one with vertices $\hat{v}_0',\hat{v}_0'',\hat{v}_1',\hat{v}_2',\hat{v}_3',\hat{v}_3''$ and with sets $A'$ and $B'$, we have set the following distances.
\begin{itemize}
...
\end{itemize}
 ...
 Next, consider the alignment gadget. Let $x_1$ belong to the edge selection gadget of color combination $i,j$ and let $w\in \hat{W}$ denote the neighbor of $x_1$ in the alignment gadget. Let $\hat{v}_1$ and $\hat{v}_2$ belong to the first connector gadget of the edge selection gadget for color combination $i,j$. Let $\hat{V}$ contain all vertices $\hat{v}_1$ and $\hat{v}_2$ belonging to the other connector gadgets (different from the first one of the edge selection gadget for color combination $i,j$). 
\begin{itemize}
...
\end{itemize}
Note that using these labels, we obey the following distance constraints.
\begin{itemize}
...
For all $j\in[k]\setminus\{i\}$. Assume $j<i$, the case of $j>i$ is analogous. Let $\{w^i_{\ell'}\}=X\cap W_i$ and $\{w^j_{\ell''}\}=X\cap W_j$ and $\{w^i_{\ell'},w^j_{\ell''}\}=e^{j,i}_\ell\in F_{j,i}$. 
...
\begin{itemize}
    \item $\lambda(\{a^{i,j,\ell'''}_{5n},u^{\ell'''}_{\ell''''}\})=(i+j)\cdot (20n+6)+2\ell-2$ for all $\ell''''$ such that this edge exists.
    \item $\lambda(\{a^{i,j,\ell'''}_{1},v^i_{j-1}\})=(i+j)\cdot (20n+6)+2\ell-10n-4$.
    \item $\lambda(\{a^{i,j,\ell'''}_{\ell''},a^{i,j,\ell'''}_{\ell''+1}\})=(i+j)\cdot (20n+6)+2\ell-2(5n-\ell'')$.
    \item $\lambda(\{b^{i,j,\ell'''}_{5n},u^{\ell'''}_{\ell''''}\})=(i+j)\cdot (20n+6)+2\ell+6$ for all $\ell''''$ such that this edge exists.
    \item $\lambda(\{b^{i,j,\ell'''}_{1},v^i_{j}\})=(i+j)\cdot (20n+6)+2\ell+10n+8$.
    \item $\lambda(\{b^{i,j,\ell'''}_{\ell''},b^{i,j,\ell'''}_{\ell''+1}\})=(i+j)\cdot (20n+6)+2\ell+2(5n-\ell'')+6$.
\end{itemize}
...
we set the label of the first edge of $P$ to be a constant form $[\Delta]$.
...
Let $v_i \neq u,v$ be a vertex of interest on the path $P_{u,v}$.
...
It follows that:
 ...
 Let $t_{v_2}\in \lbrack 1,\Delta ]$ be the label of the edge $uv_2$,
    ...
    From the assumption~\ref{assumption1-FPT} we get :
	\begin{equation*}
		\delta
...
We use all of the above discussion, to prove the following lemma.
	...
	\begin{proof}
		...
		it follows by (\ref{basic-eq-1}) that $A_{t_{v_2}}(u,v_{i},P)\leq
 ...
 The next corollary follows immediately from Lemma \ref{lem-1}.
	...
	\begin{lemma}\label{lemma:FPT-twoPathsOneUnlabeledEdge}
		...
		\{v_{i}v_{i-1}\})$.
	\end{lemma}
We are now ready to prove the follwing result.
	\begin{proof}
		Let $e\in \lbrack 1,\Delta ]$ be the label of the edge $v_{i-1}v_{i}$, and
		...
		\emph{Case 1: }$t_{u_{2}}\neq t_{v_2}$\emph{.} Then Corollary \ref{cor-1}
		...
		\emph{Case 2: }$t_{u_{2}}=t_{v_2}$\emph{.} Then, it follows by (\ref%
		{basic-eq-1}) that $%
...
\todo[inline]{Check all of the above and correct it}
