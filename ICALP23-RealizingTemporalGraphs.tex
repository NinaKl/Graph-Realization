...
\begin{proof}
	We present a polynomial-time reduction from the NP-hard problem 3-SAT~\cite{Karp1972Reducibility}. Here, we are given a formula $\phi$ in conjunctive normal form, where each clause contains exactly 3 literals (with three distinct variables). We construct an instance $D^\phi$ of \deltaExact\ as follows.
...
$x x^T, x x^F, x^T x^F, x^T v$ and $ x^Fv$.
\item For each pair of variables $x,y$ in $\phi$ with $x\neq y$ we add the following four edges: 
$x^T y^T, x^Ty^F, x^Fy^T$ and $x^Fy^F$.
\item For each clause $c$ in $\phi$ we add one edge for each literal. Let $x$ appear in $c$. If $x$ appears non-negated in $c$ we add edge $c x^T$. 
If $x$ appears negated in $c$ we add edge $c x^F$.
\end{itemize}
...
For the sake of simplicity we write $D^\phi _{x,y}$ as $d(x,y)$,
where $x,y$ are two vertices. 
We start by setting the value of $d(x,y) = 1$ where $x$ and $y$ are two adjacent vertices in $G$.
\begin{itemize}
    ...
    \item For each variable $x$ in $\phi$ we specify the following distance to the super vertex $v$:
    $d(x,v)=2$. %, $d(x_2,v)=3$.

    \item For each clause $c$ in $\phi$ we specify the following distance to the super vertex $v$:
    $d(c,v)=2$
    \item Let $x$ be a variable that appears in clause $c$, then  we specify the following distance:
    $d(c,x)=2$.
    If $x$ appears non-negated in $c$ we specify the following distance:
    $d(c,x^F)=2$.
    If $x$ appears negated in $c$ we specify the following distance:
    $d(c,x^T)=2$.
    \item Let $x$ be a variable that does \emph{not} appear in clause $c$, then we specify the following distance:
    $d(c,x^T)=d(c,x^F)=2$.
\end{itemize}
All distances between non-adjacent vertex pairs that are not defined above are set to $\infty$.

This finishes the construction of an instance $D^\phi$ of \deltaExact\, which can clearly be done in polynomial time. In the remainder we show that $D^\phi$ is a yes-instance if and only if $\phi$ is satisfiable.
...
We construct a satisfying assignment for $\phi$ as follows. For each variable $x$, if $\lambda(x x^T)=\lambda(x^T v)$, then we set $x$ to \textsc{True}, otherwise we set $x$ to \textsc{False}.
...
In particular, this means that if $\lambda(x x^F)=\lambda(x^F v)$, then we set $x$ to \textsc{False}, since in this case $\lambda(x x^T)\neq\lambda(x^T v)$.
...
Then we have that $\lambda(c x^T)=\lambda(x^T v)-1$. Furthermore, since $d(c,x)=2$ we also have that $\lambda(c x^T)=\lambda(x x^T)-1$. It follows that $\lambda(x x^T)=\lambda(x^T v)$. However, this implies that $x$ is set to \textsc{True} in the satisfying assignment and thus the clause $c$ is satisfied, a contradiction. 
...
\begin{itemize}
    ...
    \item If variable $x$ is set to \textsc{True}, we set $\lambda(x^F v)=3$.
    \item If variable $x$ is set to \textsc{False}, we set $\lambda(x^T v)=3$.
...
\begin{itemize}
    ...
    If $x$ is set to \textsc{True}, then there is a temporal path from $x$ to $v$ via $x^F$ such that $\lambda(x x^F)=2$ and $\lambda(x^F v)=3$. If $x$ is set to \textsc{False}, then there is a temporal path from $x$ to $v$ via $x^T$ such that $\lambda(x x^T)=2$ and $\lambda(x^T v)=3$.

    \item For each clause $c$ in $\phi$ we have that $d(c,v)=2$:

    Since we have a satisfying assignment there is a variable $x$ appearing in $c$ that is set to a truth-value that satisfies $c$. If $x$ appears non-negated in $c$ (and hence is set to \textsc{True}), then there is a temporal path from $c$ to $v$ through $x^T$ such that $\lambda(c x^T)=1$ and $\lambda(x^T v)=2$. If $x$ appears negated in $c$ (and hence is set to \textsc{False}), then there is a temporal path from $c$ to $v$ through $x^F$ such that $\lambda(c x^F)=1$ and $\lambda(x^F v)=2$.
...
\begin{proof}
    ...
    \begin{itemize}
        ...
        \item For every $\ell\in[m]$ and every $j\in[k]\setminus\{i\}$ we have $n$ vertices $a^{i,j,\ell}_1,a^{i,j,\ell}_2,\ldots,a^{i,j,\ell}_{n}$ and $n$ vertices $b^{i,j,\ell}_1,b^{i,j,\ell}_2,\ldots,b^{i,j,\ell}_{n}$.
    ...
    For every $\ell\in[m]$, every $j\in[k]\setminus\{i\}$, and every $\ell'\in[n-1]$ we add edge $\{a^{i,j,\ell}_{\ell'},a^{i,j,\ell}_{\ell'+1}\}$ and we add edge $\{b^{i,j,\ell}_{\ell'},b^{i,j,\ell}_{\ell'+1}\}$.
    ...
    Let $1\le j<i$ (skip if $i=1$), let $e_\ell^{j,i}\in F_{j,i}$, and let $w^i_{\ell'}\in W_i$ be incident with $e_\ell^{j,i}$. Then we add edge $\{v_{j-1}^i,a^{i,j,\ell}_{1}\}$ and we add edge $\{a^{i,j,\ell}_{n},u^\ell_{\ell'-1}\}$ between $a^{i,j,\ell}_{n}$ and the vertex $u^\ell_{\ell'-1}$ of the edge selection gadget of color combination $j,i$.
    Furthermore, we add edge $\{v_{j}^i,b^{i,j,\ell}_{1}\}$ and edge $\{b^{i,j,\ell}_{n},u^\ell_{\ell'}\}$ between $b^{i,j,\ell}_{n}$ and the vertex $u^\ell_{\ell'}$ of the edge selection gadget of color combination $j,i$.

    If $1\neq i\neq k$, then we add edge $\{v^i_{i-1},\hat{u}^i_1\}$ and for all $\ell\in[n+2]$ we add edge $\{\hat{u}^i_\ell,\hat{u}^i_{\ell+1}\}$. 
    Let $e_\ell^{i,i+1}\in F_{i,i+1}$ and let $w^i_{\ell'}\in W_i$ be incident with $e_\ell^{i,i+1}$. If $i\neq 1\neq k$, then we add edge $\{\hat{u}^i_{n+3},a^{i,i+1,\ell}_{1}\}$ and edge $\{a^{i,i+1,\ell}_{n},u^\ell_{n+\ell'-1}\}$ between $a^{i,i+1,\ell}_{n}$ and the vertex $u^\ell_{n+\ell'-1}$ of the edge selection gadget of color combination $i,i+1$. If $i=1$, then we add edge $\{v^i_0,a^{i,i+1,\ell}_{1}\}$ and edge $\{a^{i,i+1,\ell}_{n},u^\ell_{n+\ell'-1}\}$ between $a^{i,i+1,\ell}_{n}$ and the vertex $u^\ell_{n+\ell'-1}$ of the edge selection gadget of color combination $i,i+1$.
Furthermore, we add edge $\{v_{i+1}^i,b^{i,i+1,\ell}_{1}\}$ and edge $\{b^{i,i+1,\ell}_{n},u^\ell_{\ell'}\}$ between $b^{i,i+1,\ell}_{n}$ and the vertex $u^\ell_{\ell'}$ of the edge selection gadget of color combination $i,i+1$.

    Let $i+1<j\le k$ (skip if $i=k$), let $e_\ell^{i,j}\in F_{i,j}$, and let $w^i_{\ell'}\in W_i$ be incident with $e_\ell^{i,j}$. Then we add edge $\{v_{j-1}^i,a^{i,j,\ell}_{1}\}$ and edge $\{a^{i,j,\ell}_{n},u^\ell_{n+\ell'-1}\}$ between $a^{i,j,\ell}_{n}$ and the vertex $u^\ell_{n+\ell'-1}$ of the edge selection gadget of color combination $i,j$.
    Furthermore, we add edge $\{v_{j}^i,b^{i,j,\ell}_{1}\}$ and edge $\{b^{i,j,\ell}_{n},u^\ell_{n+\ell'}\}$ between $b^{i,j,\ell}_{n}$ and the vertex $u^\ell_{n+\ell'}$ of the edge selection gadget of color combination $i,j$.
    ...
    \begin{itemize}
        ...
        \item Set $B$ is empty.
    \end{itemize}
    The \emph{connector gadget for the verification gadget of color $i$} has the following sets.
    \begin{itemize}
        ...
        \item Set $B$ contains all vertices vertices $v^i_\ell$ with $\ell\in\{0,1,\ldots,i-1,i+1,\ldots,k\}$ and vertex $\hat{u}^i_{n+3}$ (if exists).
    \end{itemize}
...
\begin{itemize}
    \item We set $d(v^i_j,v^i_{j'})=(2n+2)(j'-j)+2n-1$.
\end{itemize}
For all $\ell\in[n+3]$ and $i<j\le k$ we set the following.
\begin{itemize}
    \item We set $d(\hat{u}^i_{\ell},v^i_j)=(2n+2)(j-i)+2(n-\ell)+5$.
\end{itemize}
...
\begin{itemize}
    \item We set $d(w^\star,x_1)=(2n+2)(i+j)$.
    \item We set $d(w^\star,\hat{v}_1)=n^7$, $d(w,\hat{v}_2)=n^7$, $d(w,\hat{v}_1)=n^7-(2n+2)(i+j)+1$, and $d(w,\hat{v}_2)=n^7-(2n+2)(i+j)+1$.
    \item For each vertex $v\in (V^\star\cup \hat{V})\setminus (X_{i,j}\cup \{v_{i,j}^{\star\star}\})$ we set $d(w^\star,v)=n^7+2$ and $d(w,v)=n^7-(2n+2)(i+j)+3$.
\end{itemize}
...
\draw[rounded corners, dashed, gray] (-13, 5) rectangle (8, 1.5) {};
...
\node[rotate=-90] (A) at (7,-16) {verification gadget for $j$};
    ...
    \draw[edge] (UN1) --node[timelabel,sloped] {$t_3+2n+2$} (UN11);
	\draw[edge] (UN2) --node[timelabel,sloped] {$t_m+2n+2$} (UN12);
	\draw[edge, line width=1.5pt] (UN3) --node[timelabel,sloped] {$t_1+2n+2$} (UN13);
	\draw[edge] (UNM) --node[timelabel,sloped] {$t_2+2n+2$} (UN1M);
 
    \draw[edge] (U2N1) --node[timelabel,sloped] {$t_3+4n+2$} (VSS);
	\draw[edge] (U2N2) --node[timelabel,sloped] {$t_m+4n+2$} (VSS);
	\draw[edge, line width=1.5pt] (U2N3) --node[timelabel,sloped] {$t_1+4n+2$} (VSS);
	\draw[edge] (U2NM) --node[timelabel,sloped] {$t_2+4n+2$} (VSS);
...
\begin{claim}\label{claim:4}
    For all $1\le i<j\le k$ and $\ell\in[m]$ we have that $\lambda(\{x_\ell,v_{i,j}^{\star}\})=(i+j)\cdot (2n\ell +2)$, where $x_\ell$ belongs to the edge selection gadget for $i,j$.
\end{claim}
\begin{claimproof}
    We first determine the label of $\{x_1,v_{i,j}^{\star}\}$, where $x_1$ belongs to the edge selection gadget for $i,j$. Note that $x_1$ is connected to the alignment gadget. Let $w\in \hat{W}$ be the vertex of the alignment gadget that is a neighbor of $x_1$. Since $d(w^\star,x_1)=(2n+2)(i+j)$ we have that $\lambda(\{w,x_1\})=(2n+2)(i+j)$. 

    First, assume that $\lambda(\{x_1,v_{i,j}^{\star}\})<(2n+2)(i+j)$. Then there is a temporal path from $v_{i,j}^\star$ to $w$ via $x_1$. However, we have that $d(x_{i,j^\star},w)=\infty$, a contradiction.
    Next, assume that $(2n+2)(i+j)<\lambda(\{x_1,v_{i,j}^{\star}\})<n^7+2$. Then there is a temporal path from $w$ to $v_{i,j}$ via $x_1$ with duration strictly less than $n^7-(2n+2)(i+j)+3$. However, we have that $d(w,v_{i,j}^\star)=n^7-(2n+2)(i+j)+3$, a contradiction.
...
\begin{claim}\label{claim:5}
    For all $1\le i<j\le k$ there exist a permutation $\sigma_{i,j}:[m]\rightarrow [m]$ such that for all $\ell\in[m]$ we have that $\lambda(\{u_\ell^{2n},v_{i,j}^{\star\star}\})=(i+j)\cdot (2n\cdot \sigma_{i,j}(\ell) +2)+4n+2$, where $u_\ell^{2n}$ belongs to the edge selection gadget for $i,j$.
\end{claim}
\begin{claimproof}
    ...
    By \cref{claim:4} we have that $\lambda(\{x_\ell,v_{i,j}^{\star}\})=(i+j)\cdot (2n\ell +2)$. It follows that $\lambda(\{u_{\ell'}^{2n},v_{i,j}^{\star\star}\})=(i+j)\cdot (2n\ell +2)+4n+2$. We set $\sigma_{i,j}(\ell')=\ell$.
...
\begin{claim}\label{claim:6}
    For all $1\le i<j\le k$,  $\ell\in[m]$, and $\ell'\in[2n]$ we have that $\lambda(\{u^\ell_{\ell'-1},u^\ell_{\ell'}\})=(i+j)\cdot (2n\cdot \sigma_{i,j}(\ell) +2)+2\ell'$, where $u^\ell_{\ell'-1}$ and $u^\ell_{\ell'}$ belong to the edge selection gadget for $i,j$ and $\sigma_{i,j}$ is the permutation of color combination $i,j$.
\end{claim}
\begin{claimproof}
    To do.
\end{claimproof}
...
\end{claim}
\begin{claimproof}
    Let $1<i\le k$ and $\ell\in[m]$. Assume that $n^6<\lambda(\{v_0^i,a^{i,1,\ell}_{1})< n^7+2$. The there is a temporal path from $w^\star$ to $a^{i,1,\ell}_{1}$ via $v_0^i$ that arrives at $a^{i,1,\ell}_{1}$ strictly earlier than $n^7+2$. However, we have $d(w^\star,a^{i,1,\ell}_{1})=n^7+2$, a contradiction. The argument for case where $i=1$ is analogous.
\end{claimproof}
