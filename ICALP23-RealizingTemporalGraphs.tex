...
\subparagraph{Matrix $D$.} We proceed with describing the matrix $D$, of durations of fastest paths. For a more convenient presentation, we use the notation $d(v,v'):= D_{v,v'}$. For all vertices $v,v'$ that are neighbors in $G$ we have that $d(v,v')=1$ and $d(v',v)=1$.
...
\todo[inline]{HM: check whether we introduced the notation $d(u,v,P)$.}
...
At the end we have a labeling $\lambda$ of $G$, that preserves all the durations from $D$.
