...
In the case when we want to guess the fastest path from $u$, that (somehow) passes through the vertex $x$, and goes to $v$ via an edge (or a unique path), we write it as a fastest path of the form $u \leadsto x \rightarrow v$.
...
\begin{figure}[t]
		...
		%\resizebox{\linewidth}{!}{
			...
			\end{tikzpicture}%}
		\caption{Example of a guess G-8, where we guessed the fastest paths of the form $z_2 \rightarrow w \leadsto u$ (in blue) and $z_{r-1} \rightarrow z \leadsto u$ (in red), where $u$ is a vertex of interest and $S_{w,z} = (w=z_1,z_2, \dots, z_r = z)$. \label{fig:FPT-4casesGuess}}
\end{figure}
...
Denote with $P^+$ the fastest temporal path of the form $z_2 \rightarrow w \leadsto u$,
and with $P^-$ the fastest temporal path of the form $z_{r-1} \rightarrow z \leadsto u$.
