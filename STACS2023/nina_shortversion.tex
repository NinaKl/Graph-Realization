...
Determining $F$ can be done in linear time, by simply removing edges of a spanning tree (forest) from the given graph.
...
We first give an informal and very high-level description of the main ideas of the algorithm.
We then present the first part in more details and deffer the reader to the appendix for the remaining part and correctness proof.
It is well-known that the number of paths between two vertices in a static graph can be upper-bounded by a function of the feedback edge number of the graph. Intuitively, this would allow us to ``guess'' paths between a small number of vertex pairs in $G$ (the underlying graph of our instance \deltaExact). These paths would then be used to create (representatives of) fastest temporal paths among vertex pairs in $(G,\lambda)$. We would then try to use the polynomial algorithm for trees (which includes paths) to figure out the labeling $\lambda$ of the edges used by these path. An idea like this has been used for several other temporal graph problems related to the connectivity between two vertices~\cite{casteigts2021finding,FMNR22a,EMM22}.
%
...
In our case, however, this approach does not work. Mainly because of the
	%A further difficulty arises from the 
 somewhat unintuitive characteristic of fastest paths in temporal graphs.
 ...
 Besides that, it is not enough to consider fastest temporal paths among some subset of vertices, but we have to determine the fastest temporal paths between all pairs of vertices.
...
\begin{itemize}
    \item We identify a small number of ``interesting vertices'' and specific ``graph constructions'' between which we guess representative fastest paths.
    \item For each guess %(also called ``paths configuration'') 
    we create one Integer Linear Program (ILP).
    \item We show that we can determine most labels along each path up to an additive constant. We show that the number of labels we cannot determine is small. 
    \item For each additive constant and each non-determined label we introduce a variable for an Integer Linear Program (ILP).
    \item We use equality and inequality constraints in each ILP to ensure that the fastest temporal path from $u$ to $v$ is of duration $D_{u,v}$, while any other path is of duration at least $D_{u,v}$.
    \item We compute a solution to each of the ILPs (if it exists) and construct a labeling from that solution. Since each ILP has a few variables and not too many constraints, this computation can be done efficiently using Lenstra's Theorem~\cite{Lenstra1983Integer}.
    If none of the ILP instances give a positive solution, then there exists no labeling realizing given matrix.
    This is true because considering all guesses of fastest temporal paths is equivalent to exhaustively searching through all possible temporal paths between vertices.
\end{itemize}


\subsubsection{First part of the FPT algorithm}
We now present the first part of our FPT algorithm.
...
Then we determine sets $U$, of \emph{vertices of interest}, and $U^*$ of the neighbors of vertices of interest, in the following way.
...
In order to find a labeling that satisfies this property we iterate through all pairs of vertices $v_i, v_j \in V(G)$ and 
first guess the underlying path $P$ of a fastest temporal path from $v_i$ to $v_j$, which results in one equality constraint for the labels on the path $P$. 
...
It is important to point out that the number of constraint and number of variables are not related, while we introduced all of the necessary constraints, we manage to create only a small number of variables (upper-bounded by some function in $k$).
%Note that we have to do this while keeping the total number of variables upper-bounded by some function in $k$, therefore we do not 
%
...
\begin{enumerate}[G-1.]
	...
	\\
	For fixed $S_{u,v}$ and $w \in U \cup Z^*$ we have $O(k^k)$ different possible such paths, therefore we make $O(k^{k^4})$ guesses for these paths.
	%
	...
	Detailed analysis follows arguing from above (as in G-\ref{FPT:guess-splitFromAnotherSegmentAndPaths}) and is deferred to Appendix.
...
\begin{figure}[t]
	...
	\begin{subfigure}[b]{0.48\textwidth}
		...
		\caption{Example of an Inner segment guess I (G-\ref{FPT-guessFTPamongv2z2}), where we guessed the fastest temporal paths of the form $v_2 \rightarrow u \leadsto w \rightarrow z_2$ (in blue)
	...
	\begin{subfigure}[b]{0.48\textwidth}
		...
		\caption{Example of an Inner segment guess II (G-\ref{FPT:guess-uToSegmentz2}), where we guessed the fastest temporal paths of the form $w \leadsto u \rightarrow v_2$ (in blue) and $w \leadsto v \rightarrow v_{p-1}$ (in red). 
	...
	\begin{subfigure}[b]{0.48\textwidth}
		...
		\caption{Example of a Split vertex guess I (G-\ref{FPT:guess-splitFromAnotherSegmentAndPaths}), where, for a fixed vertex $v_i \in S_{u,v}$,
	...
	\begin{subfigure}[b]{0.48\textwidth}
		...
		\caption{Example of a Split vertex guess II (G-\ref{FPT:guess-splitFromUtoAnotherSegment}), where, for a vertex of interest $w$, 
			we
...
\todo{Argue why each (some) of the guesses are important}
