...
To solve \deltaExact\ correctly it is necessary that for every pair of vertices $v_i, v_j \in V(G)$ we
\begin{itemize}
...
For problem to remain feasible, one should create only a small number of guesses.
With this in mind we create the following guesses.
%It's important to mention that whenever we say that we guess the fastest temporal path between two vertices, we mean that we guess the underlying path of a representative fastest temporal path between those two vertices.
...
Unfortunately the above guesses do not provide us with enough information to be able to determine all fastest paths. For example consider the case depicted in the FIGURE TODO. 
Therefore we introduce some more guesses. To do this we have to first define the following.
